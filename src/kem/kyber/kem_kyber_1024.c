...
OQS_KEM *OQS_KEM_kyber_1024_new(void) {
	...
	kem->length_encaps_seed = OQS_KEM_kyber_1024_length_encaps_seed;
...
#if defined(OQS_ENABLE_KEM_kyber_1024_matacc)
extern int matacc_kyber1024_ref_keypair(uint8_t *pk, uint8_t *sk);
extern int matacc_kyber1024_ref_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk);
extern int matacc_kyber1024_ref_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk);
#endif
...
#endif /* OQS_DIST_BUILD */
#elif defined(OQS_ENABLE_KEM_kyber_1024_matacc)
	return (OQS_STATUS) matacc_kyber1024_ref_keypair(public_key, secret_key);
#else
	return (OQS_STATUS) pqcrystals_kyber1024_ref_keypair(public_key, secret_key);
#endif
}

OQS_API OQS_STATUS OQS_KEM_kyber_1024_encaps_derand(uint8_t *ciphertext, uint8_t *shared_secret, const uint8_t *public_key, const uint8_t *seed) {
	(void)ciphertext;
	(void)shared_secret;
	(void)public_key;
	(void)seed;
...
#endif /* OQS_DIST_BUILD */
#elif defined(OQS_ENABLE_KEM_kyber_1024_matacc)
	return (OQS_STATUS) matacc_kyber1024_ref_enc(ciphertext, shared_secret, public_key);
#else
	return (OQS_STATUS) pqcrystals_kyber1024_ref_enc(ciphertext, shared_secret, public_key);
#endif
}
...
#endif /* OQS_DIST_BUILD */
#elif defined(OQS_ENABLE_KEM_kyber_1024_matacc)
	return (OQS_STATUS) matacc_kyber1024_ref_dec(shared_secret, ciphertext, secret_key);
#else
	return (OQS_STATUS) pqcrystals_kyber1024_ref_dec(shared_secret, ciphertext, secret_key);
#endif
}
